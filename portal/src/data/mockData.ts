...
export type PolicyCategory = 'authentication' | 'credentials' | 'rate-limits' | 'content-safety' | 'routing' | 'agent-execution';
...
export const policies: Policy[] = [
  // ─── Authentication ───
  {
    id: 'policy-jwt-validation',
    name: 'JWT Token Validation',
    description: 'Validates JWT tokens on all inbound requests — checks issuer, audience, expiration, and required claims before granting access.',
  ...
  {
    id: 'policy-entra-validation',
    name: 'Microsoft Entra Token Validation',
    description: 'Validates Microsoft Entra ID tokens — tenant verification, app identity checks, scope and role enforcement.',
    category: 'authentication',
    target: 'global',
    phase: 'runtime',
    ruleCount: 3,
  ...
  // ─── Credentials ───
  {
    id: 'policy-managed-identity',
    name: 'Managed Identity for Tools',
    description: 'Agents authenticate to tools using gateway-managed identities — no direct credential exposure. Credentials scoped per namespace.',
  ...
  {
    id: 'policy-credential-rotation',
    name: 'Credential Rotation',
    description: 'Automatically rotates secrets and API keys on a 90-day cycle. Revokes previous credentials immediately after rotation.',
    category: 'credentials',
    target: 'tools',
    phase: 'runtime',
    ruleCount: 1,
  ...
  {
    id: 'policy-secret-scope',
    name: 'Secret Scope Enforcement',
    description: 'Ensures credentials are only accessible within their assigned namespace. Prevents cross-namespace credential leakage.',
    category: 'credentials',
    target: 'tools',
    phase: 'runtime',
    ruleCount: 1,
  ...
  // ─── Rate Limits & Quotas ───
  {
    id: 'policy-model-rate-limit',
    name: 'Model Request Rate Limit',
    description: 'Limits model requests per minute globally and per user. 120 RPM global, 20 RPM per user.',
  ...
  {
    id: 'policy-token-quota',
    name: 'Token Consumption Quota',
    description: 'Enforces namespace-level token budgets — monthly quota with per-user daily limits. Alert at 80% threshold.',
    category: 'rate-limits',
    target: 'models',
    phase: 'runtime',
    ruleCount: 3,
  ...
  {
    id: 'policy-tool-rate-limit',
    name: 'Tool Invocation Rate Limit',
    description: 'Limits tool calls per minute to protect backend services. 50 calls/min per tool, 10 per user per tool.',
    category: 'rate-limits',
    target: 'tools',
    phase: 'runtime',
    ruleCount: 2,
  ...
  {
    id: 'policy-tool-access-control',
    name: 'Tool Access Control',
    description: 'Restricts tool invocations based on caller identity, namespace membership, and scope claims.',
    category: 'rate-limits',
    target: 'tools',
    phase: 'runtime',
    ruleCount: 4,
  ...
  // ─── Content Safety ───
  {
    ...
    description: 'Blocks harmful, violent, sexual, and self-harm content in prompts and responses. Uses Azure Content Safety.',
  ...
  },
  {
    ...
    description: 'Scans prompts and responses for PII (SSN, credit cards, emails). Redacts sensitive data before forwarding.',
    category: 'content-safety',
    target: 'models',
    phase: 'runtime',
    ruleCount: 2,
  ...
  {
    id: 'policy-jailbreak-defense',
    name: 'Jailbreak & Prompt Injection Defense',
    description: 'Detects prompt injection, jailbreak attempts, and indirect prompt injection in tool outputs. Blocks suspicious requests.',
    category: 'content-safety',
    target: 'agents',
    phase: 'runtime',
    ruleCount: 2,
  ...
  // ─── Routing & Transformation ───
  {
    ...
    name: 'Model Routing & Fallback',
    description: 'Routes model requests by region, provider, and capacity. Automatic PTU → PAYGO fallback and cross-region failover.',
    category: 'routing',
    target: 'models',
    phase: 'runtime',
    ruleCount: 3,
  ...
  {
    id: 'policy-header-manipulation',
    name: 'HTTP Header Policy',
    description: 'Sets namespace and workload headers (x-ai-namespace, x-workload-id) on all outbound requests. Strips authorization from forwarded calls.',
    category: 'routing',
    target: 'global',
    phase: 'runtime',
    ruleCount: 2,
  ...
  {
    id: 'policy-request-forwarding',
    name: 'Request Forwarding',
    description: 'Forwards qualifying requests to downstream model endpoints with automatic retry, timeout, and circuit-breaker logic.',
    category: 'routing',
    target: 'models',
    phase: 'runtime',
    ruleCount: 2,
  ...
  // ─── Agent Execution ───
  {
    ...
    name: 'Agent Execution Limits',
    description: 'Caps agent tool calls (max 20/run), runtime (max 60s), and iteration depth (max 8). Prevents runaway agent loops.',
  ...
  },
  {
    ...
    name: 'Agent Concurrency Throttle',
    description: 'Limits concurrent agent executions per namespace and per user to prevent resource exhaustion.',
    category: 'agent-execution',
    target: 'agents',
    phase: 'runtime',
    ruleCount: 2,
  ...
  {
    id: 'policy-agent-sandbox',
    name: 'Sandbox Environment Policy',
    description: 'Relaxes token limits and allows any model in sandbox namespaces. Production namespaces enforce strict quotas.',
    category: 'agent-execution',
    target: 'agents',
    phase: 'runtime',
    ruleCount: 2,
...
export interface Namespace {
  ...
  totalAssets: number;
  policies: string[];
  members: { name: string; role: 'namespace-admin' | 'ai-developer' | 'viewer' }[];
...
export const namespaces: Namespace[] = [
  { id: 'ns-1', name: 'ai-platform', displayName: 'AI Platform', description: 'Core AI models and infrastructure shared across the organization', owner: 'Platform Team', type: 'managed', assetCount: { models: 4, tools: 0, mcpServers: 0, agents: 0, skills: 0 }, totalAssets: 4, policies: ['Token Rate Limit', 'Content Safety'], members: [{ name: 'admin@contoso.com', role: 'namespace-admin' }, { name: 'dev-sarah@contoso.com', role: 'ai-developer' }, { name: 'dev-mike@contoso.com', role: 'ai-developer' }], serviceIdentities: [{ name: 'platform-runtime', purpose: 'Model inference' }], credentials: [{ name: 'azure-openai-key', type: 'API Key', environment: 'production' }, { name: 'anthropic-key', type: 'API Key', environment: 'production' }], environment: 'production', status: 'active', createdAt: '2026-01-10T10:00:00Z' },
  { id: 'ns-2', name: 'customer-ops', displayName: 'Customer Operations', description: 'Customer-facing tools, agents, and workflows for support and success teams', owner: 'Support Team', type: 'managed', assetCount: { models: 0, tools: 1, mcpServers: 1, agents: 1, skills: 1 }, totalAssets: 4, policies: ['Tool Access Control', 'Content Safety', 'Agent Throttle'], members: [{ name: 'ops-lead@contoso.com', role: 'namespace-admin' }, { name: 'dev-jane@contoso.com', role: 'ai-developer' }, { name: 'ops-team@contoso.com', role: 'viewer' }], serviceIdentities: [{ name: 'support-agent-runtime', purpose: 'Agent execution' }, { name: 'crm-connector', purpose: 'CRM integration' }], credentials: [{ name: 'salesforce-prod', type: 'OAuth', environment: 'production' }, { name: 'salesforce-sandbox', type: 'OAuth', environment: 'sandbox' }], environment: 'production', status: 'active', createdAt: '2026-01-15T10:00:00Z' },
  { id: 'ns-3', name: 'engineering', displayName: 'Engineering', description: 'Developer tools, DevOps automation, and CI/CD integrations', owner: 'DevOps', type: 'managed', assetCount: { models: 0, tools: 2, mcpServers: 1, agents: 1, skills: 1 }, totalAssets: 5, policies: ['Tool Access Control', 'IP Allowlist'], members: [{ name: 'devops-lead@contoso.com', role: 'namespace-admin' }, { name: 'dev-alex@contoso.com', role: 'ai-developer' }], serviceIdentities: [{ name: 'devops-assistant', purpose: 'DevOps automation' }], credentials: [{ name: 'github-token', type: 'PAT', environment: 'production' }, { name: 'jira-oauth', type: 'OAuth', environment: 'production' }], environment: 'production', status: 'active', createdAt: '2026-01-20T10:00:00Z' },
  { id: 'ns-4', name: 'finance', displayName: 'Finance', description: 'Billing, payments, and financial reporting tools', owner: 'Finance Engineering', type: 'managed', assetCount: { models: 0, tools: 2, mcpServers: 0, agents: 0, skills: 1 }, totalAssets: 3, policies: ['Tool Access Control', 'Schema Validation', 'IP Allowlist'], members: [{ name: 'fin-admin@contoso.com', role: 'namespace-admin' }, { name: 'fin-dev@contoso.com', role: 'ai-developer' }], serviceIdentities: [{ name: 'billing-service', purpose: 'Payment processing' }], credentials: [{ name: 'stripe-api-key', type: 'API Key', environment: 'production' }], environment: 'production', status: 'active', createdAt: '2026-01-25T10:00:00Z' },
  { id: 'ns-5', name: 'ai-research', displayName: 'AI Research', description: 'Experimental models and research workloads', owner: 'Research Team', type: 'managed', assetCount: { models: 2, tools: 0, mcpServers: 0, agents: 0, skills: 0 }, totalAssets: 2, policies: ['Token Rate Limit'], members: [{ name: 'research-lead@contoso.com', role: 'namespace-admin' }, { name: 'dev-mike@contoso.com', role: 'ai-developer' }], serviceIdentities: [], credentials: [{ name: 'huggingface-token', type: 'API Key', environment: 'development' }], environment: 'development', status: 'active', createdAt: '2026-02-01T10:00:00Z' },
  { id: 'ns-6', name: 'sales', displayName: 'Sales', description: 'Sales intelligence, lead scoring, and CRM analytics', owner: 'Sales Ops', type: 'managed', assetCount: { models: 0, tools: 0, mcpServers: 0, agents: 1, skills: 0 }, totalAssets: 1, policies: ['Agent Throttle'], members: [{ name: 'sales-ops@contoso.com', role: 'namespace-admin' }], serviceIdentities: [{ name: 'sales-intel-agent', purpose: 'Lead analysis' }], credentials: [], environment: 'production', status: 'active', createdAt: '2026-02-05T10:00:00Z' },
  { id: 'ns-7', name: 'hr', displayName: 'Human Resources', description: 'Employee onboarding, self-service, and HR workflows', owner: 'HR Team', type: 'managed', assetCount: { models: 0, tools: 0, mcpServers: 0, agents: 1, skills: 0 }, totalAssets: 1, policies: ['Content Safety'], members: [{ name: 'hr-admin@contoso.com', role: 'namespace-admin' }], serviceIdentities: [{ name: 'hr-onboarding-agent', purpose: 'Onboarding automation' }], credentials: [], environment: 'production', status: 'active', createdAt: '2026-02-10T10:00:00Z' },
  { id: 'ns-8', name: 'communications', displayName: 'Communications', description: 'Messaging and notification integrations', owner: 'DevOps', type: 'managed', assetCount: { models: 0, tools: 1, mcpServers: 0, agents: 0, skills: 0 }, totalAssets: 1, policies: [], members: [{ name: 'devops-lead@contoso.com', role: 'namespace-admin' }], serviceIdentities: [], credentials: [{ name: 'slack-webhook', type: 'Webhook', environment: 'production' }], environment: 'production', status: 'active', createdAt: '2026-02-15T10:00:00Z' },
  { id: 'ns-9', name: 'external-data', displayName: 'External Data', description: 'Third-party data sources and external API integrations', owner: 'Data Team', type: 'managed', assetCount: { models: 0, tools: 1, mcpServers: 0, agents: 0, skills: 0 }, totalAssets: 1, policies: ['Schema Validation'], members: [{ name: 'data-lead@contoso.com', role: 'namespace-admin' }], serviceIdentities: [], credentials: [{ name: 'weather-api-key', type: 'API Key', environment: 'production' }], environment: 'production', status: 'active', createdAt: '2026-02-18T10:00:00Z' },
  { id: 'ns-10', name: 'knowledge', displayName: 'Knowledge', description: 'Enterprise knowledge management and search services', owner: 'IT Team', type: 'managed', assetCount: { models: 0, tools: 1, mcpServers: 1, agents: 0, skills: 0 }, totalAssets: 2, policies: [], members: [{ name: 'it-admin@contoso.com', role: 'namespace-admin' }, { name: 'content-team@contoso.com', role: 'viewer' }], serviceIdentities: [{ name: 'search-indexer', purpose: 'Knowledge indexing' }], credentials: [{ name: 'azure-search-key', type: 'API Key', environment: 'production' }], environment: 'production', status: 'active', createdAt: '2026-02-20T10:00:00Z' },
  { id: 'ns-11', name: 'anishta-sandbox', displayName: 'Anish — Sandbox', description: 'Personal sandbox for experimenting with models and tools', owner: 'Anish T.', type: 'personal', assetCount: { models: 1, tools: 2, mcpServers: 0, agents: 1, skills: 0 }, totalAssets: 4, policies: [], members: [{ name: 'anish@contoso.com', role: 'namespace-admin' }], serviceIdentities: [], credentials: [], environment: 'sandbox', status: 'active', createdAt: '2026-02-25T10:00:00Z' },
  { id: 'ns-12', name: 'jane-experiments', displayName: 'Jane — Experiments', description: 'Prototyping new agent workflows for customer support', owner: 'Jane D.', type: 'personal', assetCount: { models: 0, tools: 1, mcpServers: 0, agents: 1, skills: 1 }, totalAssets: 3, policies: [], members: [{ name: 'jane@contoso.com', role: 'namespace-admin' }], serviceIdentities: [], credentials: [], environment: 'sandbox', status: 'active', createdAt: '2026-03-01T10:00:00Z' },
];
...
export const raiGuardrails: RAIGuardrail[] = [
  ...
  { id: 'rai-3', name: 'PII Redaction — Input', description: 'Scans inbound prompts for PII (SSN, credit cards, emails, phone numbers) and redacts before forwarding to model.', category: 'pii-protection', target: 'input', severity: 'warn', enabled: true, namespace: 'global', appliesTo: ['models', 'agents', 'tools'], triggersToday: 34, blockedToday: 0, lastTriggered: '2026-03-03T23:01:00Z' },
  { id: 'rai-4', name: 'PII Redaction — Output', description: 'Scans model responses for inadvertently generated PII and masks it before returning to the caller.', category: 'pii-protection', target: 'output', severity: 'warn', enabled: true, namespace: 'global', appliesTo: ['models', 'agents'], triggersToday: 8, blockedToday: 0, lastTriggered: '2026-03-03T22:30:00Z' },
  { id: 'rai-5', name: 'Jailbreak Detection', description: 'Detects prompt injection and jailbreak attempts using pattern matching and classifier model. Blocks suspicious requests.', category: 'jailbreak', target: 'input', severity: 'block', enabled: true, namespace: 'global', appliesTo: ['models', 'agents'], triggersToday: 7, blockedToday: 7, lastTriggered: '2026-03-03T21:55:00Z' },
  { id: 'rai-6', name: 'Indirect Prompt Injection Guard', description: 'Scans tool outputs and grounding data for embedded instructions that could manipulate agent behavior.', category: 'jailbreak', target: 'input', severity: 'block', enabled: true, namespace: 'global', appliesTo: ['agents'], triggersToday: 3, blockedToday: 3, lastTriggered: '2026-03-03T20:40:00Z' },
  { id: 'rai-7', name: 'Groundedness Check', description: 'Validates that model responses are grounded in provided context and flags potential hallucinations.', category: 'hallucination', target: 'output', severity: 'warn', enabled: true, namespace: 'global', appliesTo: ['models', 'agents'], triggersToday: 15, blockedToday: 0, lastTriggered: '2026-03-03T23:12:00Z' },
  { id: 'rai-8', name: 'Sexual Content Filter', description: 'Blocks generation and passthrough of sexually explicit content across all model interactions.', category: 'content-safety', target: 'both', severity: 'block', enabled: true, namespace: 'global', appliesTo: ['models', 'agents'], triggersToday: 4, blockedToday: 4, lastTriggered: '2026-03-03T18:30:00Z' },
  { id: 'rai-9', name: 'Bias & Fairness Monitor', description: 'Logs outputs that exhibit demographic bias patterns. Sends alerts but does not block responses.', category: 'fairness', target: 'output', severity: 'log', enabled: true, namespace: 'global', appliesTo: ['models', 'agents'], triggersToday: 6, blockedToday: 0, lastTriggered: '2026-03-03T17:15:00Z' },
